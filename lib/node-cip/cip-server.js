...
class CIPServer extends events_1.EventEmitter {
    ...
    pageReferenceSetButtonEnabled(smartObjectId, buttonNumber, enabled) {
        if (!this.isServerConnected()) {
            debug("Cannot set page reference button enabled: server not connected");
            return false;
        }
        return this.write(cip_output_helper_1.CIPOutputHelper.pageReferenceSetButtonEnabled(smartObjectId, buttonNumber, enabled));
    }

...
class CIP extends events_1.EventEmitter {
    ...
    connect(options) {
        ...
        // Do not remove internal SocketReconnect listeners (they manage client.connected)
        // Only ensure our own listeners are attached once
        this.client.removeListener("error", this.boundOnError);
        this.client.removeListener("connect", this.boundOnConnect);
        this.client.removeListener("close", this.boundOnClose);
        this.client.removeListener("data", this.boundOnData);
        this.client.on("error", this.boundOnError);
        this.client.on("connect", this.boundOnConnect);
        this.client.on("close", this.boundOnClose);
        this.client.on("data", this.boundOnData);
    ...
    sendSmartObjectDigital(smartObjectId, join, value) {
        const buffer = _1.CIPOutputHelper.smartObjectDigital({
            smartObjectId,
            join,
            data: value,
        });
    ...
    sendSmartObjectAnalog(smartObjectId, join, value) {
        const buffer = _1.CIPOutputHelper.smartObjectAnalog({
            smartObjectId,
            join,
            data: value,
        });
    ...
    sendSmartObjectSerial(smartObjectId, join, value) {
        const buffer = _1.CIPOutputHelper.smartObjectSerial({
            smartObjectId,
            join,
            data: value,
        });
            ...
            case _1.CIP_JOIN_TYPES.SMART_OBJECT: {
                ...
                if (event) {
                    if (event.smartObjectId) {
                        debug("=> smart object %d.%d %s: %s", event.smartObjectId, event.join, event.type, event.type === "digital"
                            ? event.data
                                ? "High"
                                : "Low"
                            : event.data.toString());
                        ...
                        debug("=> smart object %d %s: %s", event.join, event.type, event.type === "digital"
                            ? event.data
                                ? "High"
                                : "Low"
                            : event.data.toString());
    ...
    parseUnicodeMessage(payload) {
        ...
        switch (payload[6]) {
            case 0x39: {
                ...
                if (event) {
                    debug("=> smart object serial %d.%d: %s", event.smartObjectId, event.join, event.data.toString());
                    this.emit("event", event);
                }
                else {
